...
// First check if the ELF file is legit
// We don't want to load an ELF file that is not for our specific machine
int check_elf_file(const char *filename)
{
    ...
    // If all checks pass, close file and return success
    fclose(file);
    printf("ELF file is valid\n");
    return 0;
}

int load_elf_file(const char *filename, uint8_t *memory, uint32_t memory_size, uint32_t *program_counter)
{
    // Open file
    FILE *file = fopen(filename, "rb");
    if (!file)
...
int main(int argc, char *argv[])
{
    ...
    if (check_elf_file(filename) != 0)
    {
        return -1;
    }

    if (load_elf_file(filename, vm.memory, SIZE_OF_MEMORY, &vm.program_counter) != 0)
    {
        return -1;
    }
    ...
    // Dump memory to a file for testing
    FILE *output_file = fopen("output.bin", "wb");
    if (output_file)
    {
        fwrite(vm.memory, 1, SIZE_OF_MEMORY, output_file);
        fclose(output_file);
    }
    else
    {
